...
return {
  {
    ...
    config = function()
      -- Have to do this to make it work (only works with lazier)
      ...
      local in_snippet = function()
        ...
        if
          pos[1] - 1 >= snip_begin_pos[1] and pos[1] - 1 <= snip_end_pos[1]
        then
          return true
        end
      end
      ...
      local confirm = function(entry)
        ...
        if entry then
          ...
          local diff_after = math.max(
            0,
            entry.replace_range['end'].character + 1
            ...
            c = function()
              ...
              if cmp.visible() then
                ...
                if #entries == 1 then
                  ...
                  local last_str = last_cmdline_entries
                      and vim.inspect(last_cmdline_entries)
                    or nil
                  if not last_str or last_str ~= entries_str then
                    cmp.select_prev_item()
                  end
                  -- Live update cmp entries while completing (Needed for 'vim._extui')
  ...
  },
  {
    ...
    config = function()
      ...
      require('tabout').setup {
        ...
        end, { expr = true, desc = 'Accept the current inline completion or tabout' })

         vim.keymap.set('i', '<c-b>', function()
          return vim.api.nvim_replace_termcodes("<Plug>(Tabout)", true, true, false)
        end, { expr = true, desc = 'tabout' })
